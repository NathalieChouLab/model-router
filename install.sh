#!/usr/bin/env bash
# Installs the model-router skill and its eight pinned agents into ~/.claude (all projects).
...
echo "Profile: $PROFILE"; grep -H '^model:\|^effort:' "$DEST"/agents/{scout,researcher,builder,tester,writer,verifier,architect,auditor}.md | sed 's|.*/agents/||' | paste - - | column -t
